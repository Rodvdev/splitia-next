...
export default function UserDetailExpensesTab({ userId }: { userId: string }) {
  ...
  const loadExpenses = async () => {
    try {
      ...
      // Filter expenses for this user (paid by)
      const userExpenses = allExpenses.filter(expense => expense.paidBy.id === userId);
  ...
  if (expenses.length === 0) {
    return <EmptyState title="No hay gastos" description="Este usuario no tiene gastos registrados" />;
  }
