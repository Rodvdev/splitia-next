...
export default function GroupDetailTasksTab({ groupId }: { groupId: string }) {
  ...
  const loadTasks = async () => {
    try {
      ...
      const groupTasks = allTasks.filter(task => task.group?.id === groupId);
      setTasks(groupTasks);
    } catch (error) {
  ...
  if (tasks.length === 0) {
    return <EmptyState title="No hay tareas" description="Este grupo no tiene tareas registradas" />;
  }
